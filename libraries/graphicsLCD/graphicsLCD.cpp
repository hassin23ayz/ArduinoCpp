...
void graphicsLCD::printInfo(int line_no, char *title, char *value, char *unit)
{
  graphicsLCD::line(line_no);
  graphicsLCD::print(title);
  graphicsLCD::print(value);
  graphicsLCD::print(unit);  
}

void graphicsLCD::clearLine(int line_no)
{
  graphicsLCD::line(line_no);
  graphicsLCD::print("                ");
}
 ...
 void graphicsLCD::print(char *characters)
{
 ...
 void graphicsLCD::print(char *characters, byte width, byte height)
{
  while (*characters)
  {
    graphicsLCD::character(*characters++, width ,height);
  }
}
...
{
  for (int index = 0; index < 16; index++)       //character is originally 5*7  .. 1*1 is for spacing
  {
...
void graphicsLCD::character(char character, byte width, byte height)
{
  for (int index = 0; index < width; )
  {
     graphicsLCD::data(ASCII[character - 0x20][index],height);
     index+=2;  
  
  }
  graphicsLCD::data(0x00,height);  
}
...
void graphicsLCD::data(unsigned int payload, byte height)  //character is originally 5*7  .. 1*1 is for spacing
{
